...
// 插件 → 专业 Agent 包装器（P0-A）：把单个 DSH 插件包（bundle 目录）包装成标准 Agent Bundle。
// 产物与 createAgent/composeAgent 同构（agenthub.yaml + preset + profile.patch.yml），
// 插件作为 components.bundles 加入；preset 基于官方 standard 复制并替换 persona。
// 这样「装一个插件」可以升级为「生成一个专业 Agent 再一键安装」。
export function wrapPluginAsAgent({ outDir, id, name, category, publisher, plugin, bin }) {
  if (existsSync(join(outDir, 'agenthub.yaml'))) throw new Error('目标目录已有 Agent：' + outDir);
  if (!plugin || !existsSync(join(plugin, 'package.json'))) {
    throw new Error('插件目录无效（需含 package.json）：' + plugin);
  }
  const pkg = JSON.parse(readFileSync(join(plugin, 'package.json'), 'utf8'));
  const pkgName = pkg.name;
  if (!pkgName) throw new Error('插件 package.json 缺少 name 字段：' + plugin);
  const patchRel = pkg.dsh?.bundle?.patch;
  if (!patchRel || !existsSync(join(plugin, patchRel))) {
    throw new Error('插件必须声明 dsh.bundle.patch 且文件存在（否则无法作为 bundle 安装）：' + plugin);
  }
  mkdirSync(outDir, { recursive: true });

  // 复制插件 bundle 实体（installer 从 agent_dir/bundle/<last> 找 package.json）
  const bundleDst = join(outDir, 'bundle', pkgName.split('/').pop());
  mkdirSync(bundleDst, { recursive: true });
  cpSync(plugin, bundleDst, { recursive: true });
  ...
  writeFileSync(join(outDir, 'agenthub.yaml'), [
    ...
    'category: ' + category,
    'version: 0.1.0',
    'description: ' + name + '：由插件 ' + pkgName + ' 包装而成的专业 Agent。',
    'publisher:',
    '  id: ' + publisher,
    '  name: ' + publisher,
    'runtime: deepseek-harness',
    ...
    'profile:',
    '  name: ' + id,
    '  bundles: ["@deepseek-ai/dsh-base", "@deepseek-ai/dsh-web-app", "' + pkgName + '"]',
    '  patch: ./profile.patch.yml',
    'permissions:',
    '  network: []',
    '  env: []',
  ...
  // preset：复制官方 standard 组合并替换 persona（保证组合必然可挂载）
  const stdPreset = join(systemPresetsDir(bin), 'standard', 'agent.cordis.yml');
  if (!existsSync(stdPreset)) throw new Error('找不到官方 standard preset：' + stdPreset);
  const std = readFileSync(stdPreset, 'utf8');
  const persona =
    '      You are the ' + name + ' agent powered by the {{model}} model. Your working directory is {{cwd}}.\n' +
    '\n      本 Agent 由插件 ' + pkgName + ' 包装而成（' + (pkg.description || '）TODO: 写领域人设与硬性规则。');
  const composed = std.replace(
  ...
  writeFileSync(join(pdir, 'agent.cordis.yml'), composed);
  writeFileSync(join(pdir, 'preset.yml'), 'name: ' + name + '\ndescription: ' + name + '：由插件 ' + pkgName + ' 包装生成。\n');

  writeFileSync(join(outDir, 'profile.patch.yml'), ['# ' + id + ' 托管段（插件包装 Agent）。', '[]', ''].join('\n'));
  writeFileSync(join(outDir, 'README.md'), [
    '# ' + name,
    '',
    '由插件 ' + pkgName + ' 包装生成的 Agent。',
    '',
    '安装：',
    '',
    '    node ../../cli/agenthub.mjs install . --yes',
  ...
  return { outDir, plugin: pkgName, bundles: 1, presets: 1, skills: 0 };
}
