...
export function install({ agentDir, home, bin, profileName, yes = false, smoke = false, trust }) {
  ...
  try {
    ...
    step('install-bundles', () => {
      ...
      // 先解析全部 bundle 目录（只读，解析失败即中止）。
      const specs = manifest.components.bundles.map((b) => {
        ...
        return { pkgName, bundleSrc, version: b.version };
      });
      if (useOfficial) {
        // 官方路径：一次性写入全部 link: 依赖 → 单次 dsh plugin install（pnpm install + 官方对账）。
        // 注意：不用 file:——pnpm 会缓存目录内容，同名跨版本升级不刷新（e2e 实测）；
        // link: 生成符号链接，永远解析到最新目录内容。
        // 必须单次 install：pnpm 在 CI 环境默认 frozen-lockfile，分次追加依赖会导致
        // 第二次 install 锁文件过期而失败（e2e 实测 ERR_PNPM_OUTDATED_LOCKFILE）。
        // --no-frozen-lockfile：agenthub 动态管理 profile 依赖（升级/组合会重指 link: 目标），
        // 锁文件本就由本安装器维护，CI 默认冻结反而破坏升级与多 bundle 安装路径。
        for (const { pkgName, bundleSrc } of specs) {
          m.dependencies = m.dependencies ?? {};
          m.dependencies[pkgName] = 'link:' + bundleSrc;
        }
        writeManifest(pdir, m);
        const r = runDsh(bin, ['plugin', '--profile', profileName, 'install', '--no-frozen-lockfile'], { home, timeoutMs: 300000 });
        if (r.status !== 0) {
          // 保留更多行：pnpm 报错由 dsh 透传（stdio inherit），截断会吞掉真实原因
          const tail = (r.stderr || r.stdout || '').split('\n').filter(Boolean).slice(-20).join('\n');
          throw new Error('dsh plugin install 失败:\n' + tail);
        }
      } else {
        for (const { pkgName, bundleSrc, version } of specs) {
          ...
          m.dependencies = m.dependencies ?? {};
          m.dependencies[pkgName] = version ?? '0.0.0';
        }
      }
