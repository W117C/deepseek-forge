...
export function signPayload(privateKeyPem, payload) {
  // 私钥经 --key-env 环境变量传递（不进入 argv，避免本机 ps 可读；payload 走 stdin）
  return runForgeCoreJson(['sign', 'raw', '--key-env', 'FORGE_PRIVATE_KEY', '--payload-stdin'], {
    input: payload,
    env: { ...process.env, FORGE_PRIVATE_KEY: privateKeyPem },
  }).signature;
}
