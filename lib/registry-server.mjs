...
export function createRegistry({ dir, officialPublishers = ['agenthub'], operatorToken = null, requirePublisherAuth = false, artifactSecret = null, corsOrigins = null } = {}) {
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'POST' && url.pathname.match(/^\/v1\/publishers\/([^/]+)\/rotate-token$/)) {
        ...
        if (!tokenHash || sha256hex(bearer) !== tokenHash) return send(401, { error: 'publisher auth required' });
      ...
      if (req.method === 'POST' && url.pathname === '/v1/publish') {
        ...
        if (requirePublisherAuth) {
          ...
          if (!tokenHash || sha256hex(bearer) !== tokenHash) return send(401, { error: 'publisher auth required（先 agenthub publisher-register 获取令牌）' });
        }
        if (db.publishers[publisher] && db.publishers[publisher] !== publicKey) {
