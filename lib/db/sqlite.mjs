...
export function openStore({ dir, legacyJsonPath = join(dir, 'registry.json') }) {
  ...
  if (existsSync(legacyJsonPath) && !existsSync(migratedMarker)) {
    // 旧文件损坏时跳过迁移并告警，避免 openStore 直接崩溃（Registry 必须能起）
    let legacy = null;
    try {
      legacy = JSON.parse(readFileSync(legacyJsonPath, 'utf8'));
    } catch {
      console.warn(`[sqlite] 旧 registry.json 损坏，跳过迁移：${legacyJsonPath}`);
    }
    if (legacy) {
  ...
  function readState() {
    // 防御：DB 中某行 manifest/scan 损坏时用回退值，避免整个 registry 启动崩溃
    ...
    for (const r of db.prepare('SELECT * FROM packages WHERE status = ?').all('published')) {
      ...
      for (const v of db.prepare('SELECT * FROM package_versions WHERE package_id = ? AND status = ?').all(r.id, 'published')) {
        versions[v.version] = {
          ...
          manifest: safeJson(v.manifest, null), scan: safeJson(v.scan, {}), publishedAt: v.published_at, installs: v.installs,
        };
      }
      d.agents[r.id] = {
    ...
    for (const p of db.prepare('SELECT * FROM packages WHERE status = ?').all('submitted')) {
      ...
      d.pending.push({
        ...
        scan: safeJson(v.scan, {}), artifactPath: v.storage_key, signature: v.signature, sha256: v.sha256,
        manifest: safeJson(v.manifest, null), submittedAt: v.created_at,
      });
    }
    return d;
  }
